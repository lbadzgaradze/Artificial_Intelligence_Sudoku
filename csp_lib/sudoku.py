...
def flatten(seqs):
    ...
    Flattens objects in
    """
    " basically removes out-most dimension "
    return sum(seqs, [])
...
class Sudoku(CSP):
    ...
    def __init__(self, grid):
        ...
        self.rows = flatten(
            [list(map(flatten, zip(*brow))) for brow in self.bgrid])
        ...
        # iterator gets used up, therefore, by this time the size of the
        # square should be 0 since number of cells should be equal to number
        # of chars in the square
        for _ in squares:
